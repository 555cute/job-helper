import type { AppSettings, JdAnalysis, JobPost, ResumeSection, ResumeSuggestion } from '../types'
import { extractKeywords, resumeText, scoreJob } from './match'
import { uid } from './id'
...
export function parseJdText(raw: string) {
  const text = raw.trim()
  const lines = text
    .split(/\r?\n/)
    .map((l) => l.trim())
  ...
  if (companyLine) {
    company = companyLine.replace(/.*(公司|Company)[:：\s]*/i, '').slice(0, 40)
  }
...
export function analyzeJdAgainstResume(
  rawJd: string,
  resume: ResumeSection[],
  settings: AppSettings,
): JdAnalysis {
  const parsed = parseJdText(rawJd)
  const pseudo: JobPost = {
    id: uid('jd'),
    title: parsed.title,
  ...
  const missingKeywords = parsed.tags.filter((t) => !blob.includes(t.toLowerCase())).slice(0, 8)
  ...
  const pitch = buildPitch(parsed, score, settings)
...
export function buildPitch(
  ...
  const kws = (job.tags.slice(0, 3).join('、') || settings.resume.keywords || '相关技能')
  return [
    `您好，我是应聘「${job.title}」的候选人，关注到 ${job.company} 在该方向的招聘。`,
    `我有与 ${kws} 相关的落地经验，当前与岗位匹配约 ${match} 分，简历已按 JD 做了针对性优化。`,
    `方便的话想和您简单沟通下团队侧重点与下一步安排，谢谢！`,
  ].join('')
}

export function jobFromAnalysis(
  analysis: JdAnalysis,
  rawJd: string,
  source: JobPost['source'] = 'boss',
): JobPost {
  return {
    id: uid('job'),
